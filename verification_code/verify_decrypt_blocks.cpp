...
void decrypt_hier_block(ac_int<8,false> state_array[4][4],ac_int<128,false> Rkey[11],ac_channel<int> &comp_out){
...
int main() {
    ...
    ac_channel<int> col,invxorroundkey_out,data_in_key,comp_out;
    ac_channel<ac_int<8,false>> data_in_state;
    ac_int<32,false> data_in,out;
    ac_int<8,false> tmp_out[16];
    ...
    for (int i=0; i<size; i++){ //iterations
        ...
        for(int i = 0; i < 16; i++){
            Output.set_slc(i*8,plain_text[15-i]);
        }
        ...
        // std::cout<<"Output:          ";
        for(int j=0; j<16; j++){
            tmp_out[j] = count[i]<100?num[i*49+33+j]:num[i*49+33+j+(i-99)];
        }
        for(int j = 0; j < 4; j++){
            for(int i = 0; i < 4; i++){
            state_array[i][j]=tmp_out[part];    
            part++;
        ...
        //     }
        //     data_in_state.write(data_in);
        //     decrypt_hier_block(data_in_state,state_array,Rkey,comp_out);
        // }
